...
public class ChatCliente extends JFrame{
	...
	public ChatCliente() {
		...
		try {
			Remote remoteObject = Naming.lookup("Principal");

			if (remoteObject instanceof Chat) {
				chatServer = (Chat)remoteObject ;
	...
	private void textTyped(java.awt.event.KeyEvent evt) {
		...
		if (c == '\n'){
			try {
				if (firstMessage) { 
					name = textString;
					chatServer.entrar(displayChat, name);
					firstMessage = false;
				} else {
					chatServer.falar(displayChat, textString);
				}
			}
			catch (Exception ie) {				
